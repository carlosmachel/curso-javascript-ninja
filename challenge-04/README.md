...
carro.adicionarPessoas = (quantidade) => {
  let espacoDisponivel = carro.assentos - carro.quantidadePessoas;
  let pluralOuSingular = "pessoas";

  if (espacoDisponivel <= 0 && quantidade > 0) {
  ...
  if(espacoDisponivel < quantidade) {
    ...
    return `Só cabem mais ${espacoDisponivel} ${pluralOuSingular}`;
  } else {
...
carro.obterMarcaModelo(); // Esse carro é um X X.Y
